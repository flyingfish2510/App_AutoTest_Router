...
class DevicePage(BasePage):
    ...
    def _validate_page_loaded(self, timeout: int = 30):
        ...
        :return: 自身实例（支持链式调用）
        """
        logger.debug("🔍 验证设备页面加载状态")
        try:
            # 优先验证页面指示器
            self.validator.should_contain_element(self.PAGE_INDICATOR, timeout=timeout)
            logger.debug("✅ 设备页面加载成功（通过页面指示器验证）")
        except ElementNotFoundException:
            logger.warning("⚠️ 未找到页面指示器")
            raise
    ...
    def switch_to_access_device_page(self, timeout: int = 30):
        ...
        try:
            ...
            # 方式2：使用坐标点击（备用方案）
            logger.warning("⚠️ 未找到接入设备按钮，使用坐标点击")
            self.tap(0.5, 0.645)
        ...
        try:
            ...
            logger.debug("✅ 成功进入接入设备页面")
        except ElementNotFoundException as e:
            logger.error(f"❌ 进入接入设备页面失败: {e}")
            raise
